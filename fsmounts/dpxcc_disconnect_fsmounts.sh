...
logFileName="dpxcc_disconnect_fsmounts_$logFileDate.log"
PROXY_BYPASS=true
HttpsInsecure=false


show_help() {
    echo "Usage: dpxcc_disconnect_fsmounts.sh [options]"
    echo "Options:"
    ...
    echo "Example:"
    echo "dpxcc_disconnect_fsmounts.sh -m <MASKING IP> -u <MASKING User> -p <MASKING Password>"
    exit 1
}
...
disconnect_fsmounts() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="mount-filesystem/$mountId/disconnect"
    local METHOD="PUT"
    ...
    local DISCONNECT_FSMOUNT_RESPONSE
    DISCONNECT_FSMOUNT_RESPONSE=$(eval "$curl_command")

    split_response "$DISCONNECT_FSMOUNT_RESPONSE"
    check_response_error "$FUNC" "$API"

    local DISCONNECT_MOUNTID_VALUE
    DISCONNECT_MOUNTID_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.mountId')
    check_response_value "$DISCONNECT_MOUNTID_VALUE"

    local DISCONNECT_MOUNTNAME_VALUE
    DISCONNECT_MOUNTNAME_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.mountName')

    if [ ! "$DISCONNECT_MOUNTID_VALUE" == "null" ]; then
        log "FS MountId: $DISCONNECT_MOUNTID_VALUE - FS MountName: $DISCONNECT_MOUNTNAME_VALUE disconnected.\n"
    else
        log "FS MountId NOT disconnected.\n"
    fi
}
...
do
    ...
    then
        if [[ "$status" == '"ACTIVE"' ]];
        then
            disconnect_fsmounts "$mountId" "$mountName"
        fi
    fi
done < "$FsMountsFile"
