...
logFileName="dpxcc_delete_fsmounts_$logFileDate.log"
PROXY_BYPASS=true
HttpsInsecure=false


show_help() {
    echo "Usage: dpxcc_delete_fsmounts.sh [options]"
    echo "Options:"
    ...
    echo "Example:"
    echo "dpxcc_delete_fsmounts.sh -m <MASKING IP> -u <MASKING User> -p <MASKING Password>"
    exit 1
}
...
delete_fsmounts() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="mount-filesystem/$mountId"
    local METHOD="DELETE"
    ...
    log "Deleting mountId $mountId - mountName $mountName ...\n"
    build_curl "$URL_BASE" "$API" "$METHOD" "$AUTH" "$CONTENT_TYPE" "$KEEPALIVE" "$PROXY_BYPASS" "$HttpsInsecure" "$FORM" "$DATA"

    local DELETE_FSMOUNT_RESPONSE
    DELETE_FSMOUNT_RESPONSE=$(eval "$curl_command")

    split_response "$DELETE_FSMOUNT_RESPONSE"
    log "Response Code: $CURL_HEADER_RESPONSE - Response Body: $CURL_BODY_RESPONSE\n"
    log "FS MountId: $mountId - FS MountName: $mountName deleted.\n"
}
...
do
    ...
    then
        ...
        then
            delete_fsmounts "$mountId" "$mountName"
        fi
    fi
done < "$FsMountsFile"
