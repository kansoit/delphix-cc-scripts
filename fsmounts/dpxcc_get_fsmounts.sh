...
logFileName="dpxcc_get_fsmounts_$logFileDate.log"
PROXY_BYPASS=true
HttpsInsecure=false


show_help() {
    echo "Usage: dpxcc_get_fsmounts.sh [options]"
    echo "Options:"
    echo "  --fsmounts-file     -f  FS Mounts Output File                 - Default: connect_fsmounts.csv"
    ...
    echo "Example:"
    echo "dpxcc_get_fsmounts.sh -m <MASKING IP> -u <MASKING User> -p <MASKING Password>"
    exit 1
}
...
get_fsmounts() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="mount-filesystem?page_number=$page_number&page_size=$page_size"
    local METHOD="GET"
    ...
    log "Getting all fsmounts ...\n"
    build_curl "$URL_BASE" "$API" "$METHOD" "$AUTH" "$CONTENT_TYPE" "$KEEPALIVE" "$PROXY_BYPASS" "$HttpsInsecure" "$FORM" "$DATA"

    local GET_FSMOUNTS_RESPONSE
    GET_FSMOUNTS_RESPONSE=$(eval "$curl_command")

    split_response "$GET_FSMOUNTS_RESPONSE"
    check_response_error "$FUNC" "$API"

    local GET_FSMOUNTS_VALUE
    GET_FSMOUNTS_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.responseList[]')
    check_response_value "$GET_FSMOUNTS_VALUE"
}

create_fsmounts_file() {
    local csv_data
    echo "# mountId,mountName,status" >"$FsMountsFile"
    csv_data=$(echo "$CURL_BODY_RESPONSE" | jq -r ".responseList[] | [.mountId, .mountName, .status] | @csv")
    echo "$csv_data" | awk -F, -v OFS=';' '{$1=$1}1' >>"$FsMountsFile"
}
