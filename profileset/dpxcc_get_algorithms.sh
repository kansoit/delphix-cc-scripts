...
get_algorithms() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="algorithms?page_number=$page_number&page_size=$page_size"
    local METHOD="GET"
    ...
    local GET_ALGO_RESPONSE
    GET_ALGO_RESPONSE=$(eval "$curl_command")

    split_response "$GET_ALGO_RESPONSE"
    check_response_error "$FUNC" "$API"
