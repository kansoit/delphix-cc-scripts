...
add_expressions() {
    ...
    split_response "$ADD_EXPRESS_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    local ADD_EXPRESS_VALUE
    ADD_EXPRESS_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.expressionName')
    check_response_value "$ADD_EXPRESS_VALUE" "$IGN_ERROR"

    # Generate EXPRESSID_LIST
    local EXPRESSID_VALUE
    EXPRESSID_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.profileExpressionId')
...
add_profileset() {
    ...
    split_response "$ADD_PROFILE_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    local ADD_PROFILE_VALUE
    ADD_PROFILE_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.profileSetName')
    check_response_value "$ADD_PROFILE_VALUE" "$IGN_ERROR"
