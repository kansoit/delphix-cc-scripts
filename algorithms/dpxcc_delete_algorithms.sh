...
show_help() {
    ...
    echo "Options:"
    echo "  --domains-file      -d  File with Domains               - Default: dlt_algorithms.csv"
...
delete_algorithm() {
    ...
    local DLT_ALGO_RESPONSE
    DLT_ALGO_RESPONSE=$(eval "$curl_command" 2>/dev/null)

    split_response "$DLT_ALGO_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    local DLT_ALGO_VALUE
    DLT_ALGO_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.algoName')
    check_response_value "$DLT_ALGO_VALUE" "$IGN_ERROR"

    if [ ! "$DLT_ALGO_VALUE" == "null" ]; then
        log "Algorithm: $DLT_ALGO_VALUE deleted.\n"
    else
        log "Algorithm NOT deleted.\n"
    fi
}

check_packages

while getopts ":ha:i:o:x:k:" PARAMETERS; do
    case $PARAMETERS in
        h)
            show_help
            ;;
...
do
    ...
    IFS=';' read -r algoName <<< "$clean_line"

    if [[ ! "$algoName" =~ "#" ]];
    then
        delete_algorithm "$algoName"
    fi
done < "$ALGO_FILE"
