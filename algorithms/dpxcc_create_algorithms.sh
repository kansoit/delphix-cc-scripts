...
show_help() {
    ...
    echo "Example:"
    echo "dpxcc_create_algorithms.sh"
    exit 1
}
...
upload_files() {
    ...
    FILE_UPLOAD_RESPONSE=$(eval "$curl_command" 2>/dev/null)

    split_response "$FILE_UPLOAD_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    local FILE_UPLOAD_VALUE
    FILE_UPLOAD_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.filename')
    check_response_value "$FILE_UPLOAD_VALUE" "$IGN_ERROR"

    fileReferenceId=""
    fileReferenceId=$(echo "$CURL_BODY_RESPONSE" | jq -r '.fileReferenceId')
...
add_algorithm() {
    local algorithmJson="$1"

    if [ -z "$algorithmJson" ]; then
        log "Error: Could not read or parse algorithm JSON content.\n"
        return 1
    fi

    local FUNC="${FUNCNAME[0]}"
    local URL_BASE_ARG="$MASKING_ENGINE/masking/api/$apiVer" # Base URL without protocol
    local API='algorithms'
    local METHOD="POST"
    local AUTH="$AUTH_HEADER"
    local CONTENT_TYPE="application/json"
    local FORM_ARG="" # No form data for this function
    local DATA_ARG="$algorithmJson"

    local algorithmName
    algorithmName=$(echo "$DATA_ARG" | jq -r '.algorithmName')
    log "Adding Algorithm $algorithmName ...\\n"

    # Call build_curl to construct the command
    ...
    ADD_ALGO_RESPONSE=$(eval "$curl_command" 2>/dev/null) # curl_command is set by build_curl

    split_response "$ADD_ALGO_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    ADD_ALGO_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.asyncTaskId')
    check_response_value "$ADD_ALGO_VALUE" "$IGN_ERROR"

    if [ ! "$ADD_ALGO_VALUE" == "null" ]; then
        log "Algorithm: $algorithmName submitted for creation with asyncTaskId: $ADD_ALGO_VALUE.\\n"
        check_async_task_status "$ADD_ALGO_VALUE"
    else
        log "Algorithm: $algorithmName NOT submitted for creation.\\n"
    fi
}
...
check_async_task_status() {
    ...
    local API="async-tasks/$asyncTaskId"
    local METHOD="GET"
    local AUTH="$AUTH_HEADER"
    local CONTENT_TYPE="application/json"
    local FORM_ARG=""
    ...
    log "Checking status of async task $asyncTaskId ...\n"

    while true; do
        build_curl "$URL_BASE_ARG" "$API" "$METHOD" "$AUTH" "$CONTENT_TYPE" "$KEEPALIVE" "$PROXY_BYPASS" "$HttpsInsecure" "$FORM_ARG" "$DATA_ARG"
        local ASYNC_TASK_RESPONSE
        ASYNC_TASK_RESPONSE=$(eval "$curl_command" 2>/dev/null)
        split_response "$ASYNC_TASK_RESPONSE"
        check_response_error "$FUNC" "$API" "$IGN_ERROR"

        local taskStatus
        taskStatus=$(echo "$CURL_BODY_RESPONSE" | jq -r '.status')

        if [ "$taskStatus" == "SUCCEEDED" ]; then
            log "Async task $asyncTaskId succeeded.\n"
            break
        elif [ "$taskStatus" == "FAILED" ]; then
            log "Async task $asyncTaskId failed.\n"
            log "Response Body: $CURL_BODY_RESPONSE\n"
            dpxlogout
            exit 1
        else
            log "Async task $asyncTaskId is still in progress with status: $taskStatus. Waiting 5 seconds...\n"
            sleep 5
        fi
    done
}
...
get_frameworks() {
    ...
    GET_FRAMEWORK_RESPONSE=$(eval "$curl_command" 2>/dev/null)

    split_response "$GET_FRAMEWORK_RESPONSE"
    check_response_error "$FUNC" "$API" "$IGN_ERROR"

    log "CURL_BODY_RESPONSE in get_frameworks: $CURL_BODY_RESPONSE\\n"
    GET_FRAMEWORK_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.responseList')
    check_response_value "$GET_FRAMEWORK_VALUE" "$IGN_ERROR"
    log "Got frameworks...\n"
}
...
check_framework_id() {
    ...
    local currentFrameworkId
    currentFrameworkId=$(echo "$algoJson" | jq -r '.frameworkId')
    local currentPluginId
    currentPluginId=$(echo "$algoJson" | jq -r '.pluginId')

    # log "expectedFrameworkType in check_framework_id: $expectedFrameworkType\n"
    # log "allFrameworksJson in check_framework_id: $allFrameworksJson\n"
    local matchingFramework
    matchingFramework=$(echo "$allFrameworksJson" | jq --arg type "$expectedFrameworkType" '.[] | select(.frameworkName == $type)')

    if [ -n "$matchingFramework" ]; then
        local correctFrameworkId
        correctFrameworkId=$(echo "$matchingFramework" | jq -r '.frameworkId')
        local correctPluginId
...
create_fileReferenceId() {
    log "Creating $FILEREFID_NAME from log file $logFileName...\n"
    grep "Response Body" "$logFileName" | grep -oP '"fileReferenceId":"\K[^"]+' | awk '{print "\""$0"\""}' > "$FILEREFID_NAME"
    log "$FILEREFID_NAME created successfully.\n"
}

check_packages

while getopts ":ha:f:i:o:x:k:" PARAMETERS; do
    case $PARAMETERS in
        h)
            show_help
            ;;
...
check_csvf "$ALGO_FILE" "$IGN_ERROR"
...
do
    ...
    IFS=';' read -r jsonName frameworkName <<< "$clean_line"
    ...
    then
        ...
        current_algo_json=$(cat "$json_file_path")

        # Extraer el nombre del archivo de texto del campo uri
        fileName=$(echo "$current_algo_json" | jq -r '.algorithmExtension.lookupFile.uri // ""')

        # Verificar si se necesita subir un archivo
        if [ -n "$fileName" ] && [ "$fileName" != "0" ] && [[ ! "$fileName" == jar://* ]]; then
            # Determinar el fileType (asumiendo text/plain para .txt)
            fileType="text/plain"

            # Construir la ruta completa al archivo de texto
            filePathToUpload="$fileName"

            log "Uploading file: $filePathToUpload\\n"
            upload_files "$filePathToUpload" "$fileType"

            # El fileReferenceId es una variable global establecida por upload_files
            # Actualizar el JSON con el fileReferenceId real
            log "Updating JSON with fileReferenceId: $fileReferenceId\\n"
