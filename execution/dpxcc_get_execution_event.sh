...
logFileName="dpxcc_get_execution_event_$logFileDate.log"
OutputFileType="json"
OutputFileName="execution_event_$logFileDate"
PROXY_BYPASS=true
HttpsInsecure=false


show_help() {
    echo "Usage: dpxcc_get_execution_event.sh [options]"
    echo "Options:"
    ...
    echo "Example:"
    echo "dpxcc_get_execution_event.sh -m <MASKING IP> -u <MASKING User> -p <MASKING Password>"
    exit 1
}
...
get_execution_event() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="execution-events?page_number=$page_number&page_size=$page_size"
    local METHOD="GET"
    ...
    log "Getting Execution Events ...\n"
    build_curl "$URL_BASE" "$API" "$METHOD" "$AUTH" "$CONTENT_TYPE" "$KEEPALIVE" "$PROXY_BYPASS" "$HttpsInsecure" "$FORM" "$DATA"

    local GET_EXEVENT_RESPONSE
    GET_EXEVENT_RESPONSE=$(eval "$curl_command")

    split_response "$GET_EXEVENT_RESPONSE"
    check_response_error "$FUNC" "$API"

    local GET_EXEVENT_VALUE
    GET_EXEVENT_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.responseList[]')
    check_response_value "$GET_EXEVENT_VALUE"
}
...
cvt_output() {
    ...
    if [ "$OutputFileType" == "csv" ]; then
        ...
        echo "executionEventId;executionId;eventType;severity;cause;count;timeStamp;executionComponentId;maskedObjectName;algorithmName;exceptionType;exceptionDetail" > "$CsvFileName"
