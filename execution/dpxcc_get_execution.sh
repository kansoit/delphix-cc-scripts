...
logFileName="dpxcc_get_execution_$logFileDate.log"
OutputFileType="json"
OutputFileName="execution_$logFileDate"
PROXY_BYPASS=true
HttpsInsecure=false


show_help() {
    echo "Usage: dpxcc_get_execution.sh [options]"
    echo "Options:"
    ...
    echo "Example:"
    echo "dpxcc_get_execution.sh -m <MASKING IP> -u <MASKING User> -p <MASKING Password>"
    exit 1
}
...
get_execution() {
    ...
    local FUNC="${FUNCNAME[0]}"
    local URL_BASE="$MASKING_ENGINE/masking/api/$apiVer"
    local API="executions?page_number=$page_number&page_size=$page_size"
    local METHOD="GET"
    ...
    log "Getting Executions ...\n"
    build_curl "$URL_BASE" "$API" "$METHOD" "$AUTH" "$CONTENT_TYPE" "$KEEPALIVE" "$PROXY_BYPASS" "$HttpsInsecure" "$FORM" "$DATA"

    local GET_EXECUTION_RESPONSE
    GET_EXECUTION_RESPONSE=$(eval "$curl_command")

    split_response "$GET_EXECUTION_RESPONSE"
    check_response_error "$FUNC" "$API"

    local GET_EXECUTION_VALUE
    GET_EXECUTION_VALUE=$(echo "$CURL_BODY_RESPONSE" | jq -r '.responseList[]')
    check_response_value "$GET_EXECUTION_VALUE"
}
...
cvt_output() {
    ...
    if [ "$OutputFileType" == "csv" ]; then
        ...
        echo "executionId;jobId;status;rowsMasked;rowsTotal;bytesProcessed;bytesTotal;startTime;endTime;submitTime" > "$CsvFileName"
